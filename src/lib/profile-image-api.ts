  ...
  async checkAuthenticationStatus(): Promise<{
    ...
    if (isFeatureEnabled("useSupabaseIndicators") && supabase) {
      ...
      if (authError || !supabaseUser) {
        console.warn("⚠️ Usuário não autenticado no Supabase. Usando localStorage como fallback.");
      ...
      if (supabaseUser.id !== userId) {
        console.warn("⚠️ User ID mismatch - Supabase User:", supabaseUser.id, "Requested User:", userId);
...
if (typeof window !== 'undefined') {
  ...
  window.migrateImages = async () => {
    console.log("🔄 Iniciando migra��ão de imagens...");
    try {
